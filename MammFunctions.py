...
def generatePage(facility_name, facility_startrow, facility_endrow):
    ...
    def fetchData():
        result = service.spreadsheets().values().get(
                                        ...
                                        range='CISCurrentFacilityValues!A1:BO43'
                                        ).execute()
    ...
    if dfit.equals(dfgo) == False:
        ...
        service.spreadsheets().values().update(
                                        spreadsheetId=spreadsheetId, 
                                        range=f'CISCurrentFacilityValues!A{outputStart}:BO{outputEnd}',
    ...
    if submissionButton:
        if len(auditUser) >0:
            ...
            service.spreadsheets().values().update(
                                                    spreadsheetId=spreadsheetId, 
                                                    range=f'CISCurrentFacilityValues!A{outputStart}:BQ{outputEnd}',
